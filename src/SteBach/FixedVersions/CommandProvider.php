        		...
        		foreach ($this->rawData[$checkkey] as $name => $version) {
    				if (isset($this->localPackages[$name])) {
    					if ($version != $this->localPackages[$name]) {
			        		...
			        		$output->writeln("  - package \"" . $name . "\" was changed  to version \"" . $this->localPackages[$name] . "\"");
    					} else {
			        		$output->writeln("  - package \"" . $name . "\" was already set to version \"" . $version . "\"");
    					}
    				} else {
		        		$output->writeln("  - package \"" . $name . "\" not found - version remains \"" . $version . "\"");
    				}
        		}
        	}
        }
